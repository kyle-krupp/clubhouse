...
const App = () => {
  ...
  const [selectedPlayer, editPlayer] = useState({});
  const [isFetching, fetched] = useState(false);
  const [isAddPlayerModalVisible, closeAddPlayerModal] = useState(false);
  const [isEditPlayerModalVisible, closeEditPlayerModal] = useState(false);
